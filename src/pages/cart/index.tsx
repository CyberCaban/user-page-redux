  ...
  return (
    <Layout>
        ...
        {items.map(
          (item: {
            ...
            <li key={item.id}>
              ...
              <input
                onChange={(e) =>
                  handleAmount(item.text, parseInt(e.target.value))
                }
                value={item.amount}
              ...
              <DeleteOutlined
                onClick={() => handleAmount(item.text, 0)}
      ...
      <h2>
        Итого{" "}
        {items.reduce(
          (accumulator: number, currValue: { price: number; amount: number }) =>
            accumulator + currValue.price * currValue.amount,
          0
        )}
      </h2>
    </Layout>
